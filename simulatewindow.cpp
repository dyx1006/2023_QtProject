...
void SimulateWindow::playGame()
{
    ...
    connect(&m_Timer,&QTimer::timeout,[=](){
        if(judge_stop)
        {
            ...
            t = t % int((light1 + light2 + 4000) / simulate_speed);
            if(t <= light1 / simulate_speed) light = 0;
            else if((light1 + 2000) / simulate_speed <= t && t <= (light1 + light2 + 2000) / simulate_speed) light = 1;
            else
            {
                light = 2;
                if(t < (light1 + 2000) / simulate_speed) judge_yellow_light = false;
                else judge_yellow_light = true;
            }
            //            //第二种情况：调整t，但这样的话不能改变车辆生成速度和转向速度
            //            t *= simulate_speed; // 倍速
            ...
            if(ran < car_stream_1 * simulate_speed)
                build_left_to_right_Cars();
            else if(ran < (car_stream_1 + car_stream_2) * simulate_speed)
                build_turn_right_Cars();
            else if(ran < (car_stream_1 + car_stream_2 + car_stream_3) * simulate_speed)
                build_right_to_left_Cars();
            else if(ran < (car_stream_1 + car_stream_2 + car_stream_3 + car_stream_4) * simulate_speed)
...
void SimulateWindow::updateCarSpeedAndTimer()
{
    ...
    for(int i = 0; i < num; i++)
    {
        cars_turn_right[i].setSpeed(0,-3*simulate_speed);
    }
    num = cars_turned_right.count();
    for(int i = 0; i < num; i++)
    {
        cars_turned_right[i].setSpeed(3*simulate_speed,0);
    }
    num = cars_lef_to_right.count();
    for(int i = 0; i < num; i++)
    {
        cars_lef_to_right[i].setSpeed(3*simulate_speed,0);
    }
    num = cars_right_to_left.count();
    for(int i = 0; i < num; i++)
    {
        cars_right_to_left[i].setSpeed(-3*simulate_speed,0);
    }
    num = cars_turn_left.count();
    for(int i = 0; i < num; i++)
    {
        cars_turn_left[i].setSpeed(0,-3*simulate_speed);
    }
    num = cars_turned_left.count();
    for(int i = 0; i < num; i++)
    {
        cars_turned_left[i].setSpeed(-3*simulate_speed,0);
    }
}
